...
from utils import MagprintDataset4Pretrain, load_pretrain_data_config, get_device, handle_argv, \
...
def fetch_setup(args, output_embed):
    ...
    model = MagprintModel4Pretrain(model_cfg, output_embed=output_embed)
...
def generate_embedding_or_output(args, save=False, output_embed=True):
    ...
    def func_forward(model, batch):
        ...
        seq_recon_numpy = seq_recon.numpy()
        mask_seqs_numpy = mask_seqs.numpy()
...
def load_embedding_label(model_file, dataset, dataset_version):
    ...
    # label_name = 'act_label_' + dataset_version  # 这里只预测动作的标签
    embed = np.load(os.path.join('embed', embed_name + '.npy')).astype(np.float32)
    labels = np.load(os.path.join('dataset', 'labels_numpy', label_name + '.npy')).astype(np.float32)
    return embed, labels
...
if __name__ == "__main__":
    ...
    # 输出decoder之前的部分，即transformer——encoder的输出
