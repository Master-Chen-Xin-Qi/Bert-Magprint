...
USERS = ['fangliang', 'lanqing', 'panhao', 'wangzhong', 'weilun', 'yeqi', 'zhoujie']
SIGMA = 1  # 高斯滤波的sigma参数
WINDOW_LEN = 240  # 时间窗的长度(除以10之后)
MODEL_FOLDER = './config'  # 保存归一化函数的地方
...
def create_io_config(args, dataset_name, version, pretrain_model=None, target='pretrain'):
    data_path = os.path.join('dataset', 'users_numpy', 'data_' + version + '.npy')
    label_path = os.path.join('dataset', 'labels_numpy',  'label_' + version + '.npy')
    args.data_path = data_path
    args.label_path = label_path

    save_path = os.path.join('dataset/saved', target + "_" + dataset_name + "_" + version)  # + "_temp"
    ...
    if pretrain_model is not None:
        if target.count('_') > 2: # bert_classifier
            model_path = os.path.join('dataset/saved', 'pretrain_' + target.split('_')[2] + "_" + dataset_name + "_" + version, pretrain_model)
        else:
